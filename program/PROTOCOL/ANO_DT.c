/**
  ...
  ******************************************************************************
  需配置函数：
  1.static void ANO_DT_Send_Data(uint8_t *dataToSend, uint8_t length)
  2.中断函数，在函数中写入
	uint16_t res = 0;
...
void Save_PID1_3(void);
void Save_PID4_6(void);
void Save_PID7_9(void);
void Save_PID10_12(void);
void Save_PID13_15(void);
void Save_PID16_18(void);
...
static void ANO_DT_Data_Receive_Anl(uint8_t *data_buf, uint8_t num)
{
	...
	if (*(data_buf + 2) == 0X10)	//写入修改 1~3 组PID				//PID1
	{
		...
		Save_PID1_3();
		//Velo_PID_Cal[0].Target = ctrl_1.PID[PID1].kp;	
...
static void ANO_DT_Send_Data(uint8_t *dataToSend, uint8_t length)
{
	while (length--)
	{
		...
		Usart_SendByte(USART1, *dataToSend);  //串口发送字符，需配置
		dataToSend++;
	}
}
...
/******************* 2019.10.04 by Liu CX *******END OF FILE************/
